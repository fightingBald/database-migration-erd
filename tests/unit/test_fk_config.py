...
def test_lookup_rejects_ambiguous_or_wrong_qualified_table(tmp_path):
    schema = {
    ...
    for table_name in ("a", "missing.a"):
        ...
        apply_foreign_key_config(
            schema, entries, config_source=source, failures=failures
        )
        assert failures
        assert not any(t.foreign_keys for t in schema.values())


def test_lookup_rejects_unknown_target_or_column(tmp_path):
...
def test_valid_config_deduplicates_and_accepts_unambiguous_suffix(tmp_path):
    ...
    apply_foreign_key_config(schema, entries, config_source=source, failures=failures)
    assert not failures
