...
from erd_generator.d2_layout import connected_components, pack_sizes, plan_layout
...
def test_cluster_supernodes_pack_by_estimated_area_without_empty_columns():
    clusters = [
        (("catalog",), 420, 180),
        (("orders",), 360, 520),
        (("identity",), 300, 260),
    ...
    original = list(clusters)
    packed = pack_sizes(clusters)
    assert all(column for column in packed)
    assert sorted(key for column in packed for key in column) == sorted(
        key for key, _, _ in clusters
    )
